...
{ //convert to pcl(if its point cloud sensor msg then it will be converted to point cloud pcl msg) data type
  ...
  else
  {
    ...
    waypoints[tour_length-1][0]=15;
    waypoints[tour_length-1][1]=15; 
    waypoints[tour_length-1][2]=-8;
...
{ 
  ...
  ros::Subscriber odometry_sub = n.subscribe<nav_msgs::Odometry>("quad_sim_sysid_node/pose", 1, GetOdom);
  ...
  {  
    ...
    for(i = 0; i < new_tour.size(); i++)
    { 
      waypoint_marker_msg.points.at(i).x = waypoints[new_tour.at(i)][0];       
      waypoint_marker_msg.points.at(i).y = waypoints[new_tour.at(i)][1];      
      waypoint_marker_msg.points.at(i).z = waypoints[new_tour.at(i)][2]; 
